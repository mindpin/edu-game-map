  ...
  class Map
    module JsonMethods
      ...
      def _set_map_id_and_name
        json_map_id   = _json_hash["id"]
        json_map_name = _json_hash["name"]
      ...
      def _parse_json_hash
        @begin_nodes || begin
          id__node_hash = {}
          _json_hash["nodes"].each do |node_hash|
            id__node_hash[node_hash["id"]] = EduGameMap::Node.new(self, node_hash)
          end
          _json_hash["relations"].each do |relation|
